...
const Users = () => {
  ...
  const filterUsers = () => {
    ...
    if (selectedTab === 'Applicants Who Applied For Verification') {
      filtered = filtered.filter(user =>
        verificationRequests.some(request => user.userType === "applicant" && request.addedBy === user.uid)
      );
    } else if (selectedTab === 'Pending Applicants') {
      filtered = filtered.filter(user =>
        verificationRequests.some(request => user.userType === "applicant" && request.addedBy === user.uid && request.status === 'Pending')
      );
    } else if (selectedTab === 'Verified Applicants') {
      filtered = filtered.filter(user =>
        verificationRequests.some(request => user.userType === "applicant" && request.addedBy === user.uid && request.status === 'Verified')
      );
    } else if (selectedTab === 'Rejected Applicants') {
      filtered = filtered.filter(user =>
        verificationRequests.some(request => user.userType === "applicant" && request.addedBy === user.uid && request.status === 'Rejected')
      );
    } else if (selectedTab === 'Applicants') {
      filtered = filtered.filter(user => user.userType === "applicant");
    } else if (selectedTab === 'Visa Officers') {
      filtered = filtered.filter(user => user.userType === "visa officer");
    } else if (selectedTab === 'Super Admins') {
      filtered = filtered.filter(user => user.userType === "super admin");
    }
  ...
  const handleEdit = (user) => {
    ...
    if (user.userType === "applicant" || user.userType === "visa officer" || user.userType === "super admin") {
  ...
  return (
    <section className="container px-4 mx-auto">
      ...
      <div className="flex mt-6 gap-x-4 w-full overflow-x-auto">
        {['All Users', 'Applicants', 'Visa Officers', 'Super Admins', 'Applicants Who Applied For Verification', 'Pending Applicants', 'Verified Applicants', 'Rejected Applicants'].map(tab => (
          <button
            key={tab}
                ...
                <tbody className="bg-white divide-y divide-gray-200 dark:divide-gray-700 dark:bg-gray-900">
                  {currentUsers.map(user => (
                    <tr key={user.id}>
                      ...
                      <td className="px-4 py-4 text-sm text-gray-500 dark:text-gray-300 whitespace-nowrap">{user.userType === "applicant" ? ('Applicant') : user.userType === "visa officer" ? ('Visa Officer') : user.userType === "mini admin" ? 'Mini Admin' : user.userType === "super admin" ? 'Super Admin' : 'None'} </td>
