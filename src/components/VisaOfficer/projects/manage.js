...
const Manage = () => {
    ...
    useEffect(() => {
      const fetchProjects = async () => {
        try {
          // Fetch all projects
          const querySnapshot = await getDocs(collection(db, 'evisaApplication'));
        ...
        //  const userProjects = allProjects.filter(project => project.addedBy === id);
          ...
          allProjects.sort((a, b) => a.fullName.localeCompare(b.fullName));
    ...
    const handleDelete = async () => {
      ...
      try {
        await deleteDoc(doc(db, 'evisaApplication', projectToDelete.id));
        setProjects(projects.filter(project => project.id !== projectToDelete.id));
        toast.success("You have successfully deleted the e-visa.")
        closeDeleteModal();
      } catch (error) {
    ...
    <div class="sm:flex sm:items-center sm:justify-between">
        <div>
            <div class="flex items-center gap-x-3">
                <h2 class="text-lg font-medium text-gray-800 dark:text-white">Manage E-Visa Applications</h2>

                <span class="px-3 py-1 text-xs text-blue-600 bg-blue-100 rounded-full dark:bg-gray-800 dark:text-blue-400">{projects.length} E-Visa Applications</span>
            </div>

            <p class="mt-1 text-sm text-gray-500 dark:text-gray-300">Browse through all applicants&apos; E-Visa applications</p>
        </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead class="bg-gray-50 dark:bg-gray-800">
                            <tr>
                                ...
                                <th scope="col" class="px-4 py-3.5 text-sm font-normal text-left rtl:text-right text-gray-500 dark:text-gray-400">Travel Duration</th>
                           ...
                           {currentProjects.map((project) => (
                            <tr key={project.id} onClick={() => openModal(project)} className="cursor-pointer">
                                <td class="px-4 py-4 text-sm font-medium whitespace-nowrap">
                                    <div>
                                        <h2 class="font-medium text-gray-800 dark:text-white ">{project.fullName.slice(0,6)}...</h2>
                                        <p class="text-sm font-normal text-gray-600 dark:text-gray-400">{project?.residentialCountry}</p>
                                    </div>
                                </td>
      ...
      project.status === 'Approved'
        ...
        : 'text-emerald-500 bg-emerald-100/60 dark:bg-gray-800'
                                ...
                                <td class="px-4 py-4 text-sm whitespace-nowrap">
                                    <div>
                                        <h4 class="text-gray-700 dark:text-gray-200">{project.email}</h4>
                                        <p class="text-gray-500 dark:text-gray-400">{project.description && project.description.slice(0,8)}...</p>
                                    </div>
                                </td>
                                <td class="px-4 py-4 text-sm whitespace-nowrap">
                                    <div class="flex items-center">
                                    <h4 class="text-gray-700 dark:text-gray-200">{project.travelDuration}</h4>
                                    </div>
                                </td> 

                                <td class="px-4 py-4 text-sm whitespace-nowrap">
                                <h4 class="text-gray-700 dark:text-gray-200">{project.createdAt}</h4>
                                  {/**
