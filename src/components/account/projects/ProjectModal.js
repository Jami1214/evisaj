    ...
    <div className={`fixed top-0 left-0 z-20 h-screen w-screen bg-[#242d34bb] overflow-y-auto shadow-md`} onClick={closeModal}>
      <div className='bg-white w-[350px] md:w-[650px] text-black absolute left-[50%] translate-x-[-50%] mt-[40px] p-4 rounded-[20px] overflow-y-auto no-scrollbar h-[380px] md:h-[450px]' onClick={(e) => e.stopPropagation()}>
        <div className='flex justify-between items-center mb-4'>
          <div className='text-xl font-bold'>E-Visa Application Status</div>
        ...
        <div>
          ...
          <div>
            ...
            <textarea className='w-full border rounded p-2' value={applicationRemarks} disabled></textarea>
          </div>
         
          <div>
             <label className='block text-sm font-medium'>Status</label>
             <button
      className={`px-4 py-2 w-full text-center text-black font-bold rounded-lg shadow-md transition duration-300 ease-in-out transform hover:scale-105 ${bgColor}`}
    >
      {status}
    </button>
          </div>
