...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchProjectsAndDonations = async () => {
      try {
        if (userDetails?.userType === "applicant") {
          // Fetch projects by the current creator
          const projectsQuery = query(collection(db, 'evisaApplication'), where('addedBy', '==', id));
              ...
              <div className="bg-blue-600 p-8 md:p-12 lg:px-16 lg:py-24">
                <div className="mx-auto max-w-xl text-center">
                  <h2 className="text-2xl font-bold text-white md:text-3xl">
                    {userDetails?.userType === "applicant"  ? 'Total E-Visas' : 'Total Donations'}
                  </h2>
                  ...
                  <p className="hidden text-white/90 sm:mt-4 sm:block">
                    ...
                    {userDetails.userType === "applicant" && (
                      <>Welcome to your dashboard! Browse through your e-visa applications, View your e-visa status and manage your profile efficiently.</>
                    )}
                  </p>
            ...
            <div className="bg-white p-6 rounded-lg shadow-md">
              <div>
                <h3 className="text-xl font-bold mb-4">
                  {userDetails?.userType === "applicant" ? 'Projects Summary' : 'Donations Summary'}
                </h3>
                    ...
                    projects.map((project) => (
                      <div key={project.id} className="bg-white p-4 rounded-lg shadow-md cursor-pointer">
                        <div class="h-38 flex flex-col justify-center items-center bg-[url('https://preline.co/assets/svg/examples/abstract-bg-1.svg')] bg-no-repeat bg-cover bg-center rounded-t-xl">
                          <img src={project.passportPic} alt="project image" className='w-full h-38 rounded-md' />
                        </div>
                        <h4 className="text-lg font-semibold mb-2">{project.applicationNumber}</h4>
                        <p className="text-gray-600 mb-2">{project.travelPurpose}</p>
                        ...
                        <p className="text-gray-600 mb-2">{project.createdAt}</p>
